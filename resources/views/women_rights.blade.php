    ...
    <div class="position-relative">
        <ul class="nav nav-tabs container contact_us fixed-nav" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="women_rights_news nav-link custom_list_group_action" id="section1-tab" data-bs-toggle="tab"
                    data-bs-target="#section1" type="button" role="tab" aria-controls="section1"
                    aria-selected="true">{{ __('main.news') }}</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="women_rights_documents nav-link custom_list_group_action" id="section2-tab" data-bs-toggle="tab"
                    data-bs-target="#section2" type="button" role="tab" aria-controls="section2"
                    aria-selected="false">{{ __('main.legal_documents') }}</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="women_rights_advice nav-link custom_list_group_action" id="section3-tab" data-bs-toggle="tab"
        ...
        <div class="tab-content" id="myTabContent">
            <div class="tab-pane container_section fade show active" id="section0" role="tabpanel"
                ...
                <div class="d-flex flex-column directions_applications">
                    ...
                    <div class="soldier_rights_text font_reset mb-5">{!! $text_4 !!}</div>
                </div>
            </div>
                ...
                <div class="directions_applications">
                    ...
                    @foreach ($staticTexts as $text)
                        @if ($text->page == 'women_rights_advice')
                            @if ($loop->first)
                                <p class="soldier_rights_header_text">{!! $text->{'content_' . app()->getLocale() } ? $text->{'content_' . app()->getLocale() } : $text->content_am !!}</p>
                                <div class="soldier_rights_header_date d-flex">
                                    <img src="{{ asset('img/calendar.png') }}" alt="calendar">
                                    <p>{{ $text->created_at }}</p>
                                </div>
                            @endif
                            <p class="soldier_rights_text">{!! $text->{'content_' . app()->getLocale() } ? $text->{'content_' . app()->getLocale() } : $text->content_am !!}</p>
                        @endif
                    @endforeach
