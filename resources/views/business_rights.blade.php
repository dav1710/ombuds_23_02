...
@section('content')
    <div class="position-relative">
        <ul class="nav nav-tabs container contact_us" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="business_rights_news nav-link custom_list_group_action" id="section1-tab" data-bs-toggle="tab"
                    data-bs-target="#section1" type="button" role="tab" aria-controls="section1"
                    aria-selected="true">{{ __('main.news') }}</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="business_rights_documents nav-link custom_list_group_action" id="section2-tab" data-bs-toggle="tab"
        ...
        <div class="tab-content" id="myTabContent">
            <div class="tab-pane container_section fade show active" id="section0" role="tabpanel"
                ...
                <div class="d-flex align-items-baseline contact_us_action">
                    ...
                    <div class="contact_us_action_title">
                        <p>{{ __('main.protection_rights') }}</p>
                    </div>
                </div>
                <div class="d-flex flex-column directions_applications torture_directions_applications">
                    @foreach ($pages as $page)
                        @if ($page->page == 'business_rights_main')
            ...
            <div class="tab-pane container_section directions_applications fade" id="section1" role="tabpanel"
                ...
                <div class="directions_applications d-flex flex-column">
                    @foreach ($documents as $item)
                        @foreach ($item->tabs as $tab)
                            @if ($tab->name == 'business_rights_pdf')
    ...
    @foreach ($mediaFile as $item)
        @foreach ($item->tabs as $tab)
            @if ($tab->name == 'business_rights_main')
                <audio
