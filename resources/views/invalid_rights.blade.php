...
@section('content')
    <div class="soldier_rights  position-relative">
        <ul class="nav nav-tabs container contact_us fixed-nav" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="invalid_rights_news nav-link custom_list_group_action" id="section1-tab" data-bs-toggle="tab"
                    data-bs-target="#section1" type="button" role="tab" aria-controls="section1"
                    aria-selected="true">{{ __('main.news') }}</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="invalid_rights_documents nav-link custom_list_group_action" id="section2-tab" data-bs-toggle="tab"
                    data-bs-target="#section2" type="button" role="tab" aria-controls="section2"
                    aria-selected="false">{{ __('main.legal_documents') }}</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="invalid_rights_advice nav-link custom_list_group_action" id="section3-tab" data-bs-toggle="tab"
        ...
        <div class="tab-content" id="myTabContent">
            <div class="tab-pane container_section fade show active" id="section0" role="tabpanel"
                ...
                <div class="tab-header">
                    <div class="d-flex align-items-baseline contact_us_action">
                        ...
                        <div class="contact_us_action_title">
                            <p>{{ __('main.invalid_rights') }}</p>
                        </div>
                    </div>
                </div>
                <div class="d-flex flex-column directions_applications">
                    @foreach ($pages as $page)
                    @if ($page->page == 'invalid_rights_main')
                ...
                <div class="soldier_rights_text font_reset mb-5">{!! $text_2 !!}</div>
                </div>
            </div>
                ...
                <div class="directions_applications d-flex flex-column">
								@foreach($documents as $item)
									@foreach($item->tabs as $tab)
										@if($tab->name == "invalid_rights_pdf")
											...
											<div class="report_news d-flex flex-column">
												<a class="report_pdf report_pdf_news" href="{{ asset('uploads/documents/' . ($item->{'document_' . app()->getLocale() } ? $item->{'document_' . app()->getLocale() } : $item->document_am)) }}" class="mb-3" download="{{ $item->{'title_' . app()->getLocale() } ? $item->{'title_' . app()->getLocale() } : $item->title_am }}">
												    <img src="{{ asset('img/icons/' . $extension . '.png') }}" alt="{{ $extension }}">{{ $item->{'title_' . app()->getLocale() } ? $item->{'title_' . app()->getLocale() } : $item->title_am }}
												</a>
											</div>
										@endif
									@endforeach
                ...
                <div class="directions_applications">
												@foreach($mediaFile as $item)
													@foreach($item->tabs as $mediaFile_tab)
														@if($mediaFile_tab->name == "women_rights_advice")
										...
										@if($text->page == "women_rights_advice")
											@if($loop->first)
    ...
    @foreach ($mediaFile as $item)
        @foreach ($item->tabs as $tab)
            @if($tab->name == 'invalid_rights_main')
